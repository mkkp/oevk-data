...
def transform_all_optimized(db_connection: duckdb.DuckDBPyConnection, run_tag: str, chunk_size: int = 50000, parallel: bool = False, db_path: str = '') -> None:
...
def transform_addresses_optimized(db_connection: duckdb.DuckDBPyConnection, run_tag: str, chunk_size: int = 50000) -> None:
...
def transform_addresses_parallel(db_connection: duckdb.DuckDBPyConnection, run_tag: str, chunk_size: int = 50000, db_path: str = '') -> None:
    ...
    logger.info(f"Processing {total_count:,} addresses in {total_chunks} parallel chunks of {chunk_size:,}")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for chunk_num in range(total_chunks):
            offset = chunk_num * chunk_size
            future = executor.submit(process_chunk_parallel, db_path, run_tag, chunk_num, total_chunks, offset, chunk_size, total_count, start_time)
            futures.append(future)
        
        # Wait for all chunks to complete with timeout
        logger.info(f"Waiting for {len(futures)} chunks to complete...")
        completed, not_completed = concurrent.futures.wait(futures, timeout=300)  # 5 minute timeout
        
        if not_completed:
            logger.warning(f"{len(not_completed)} chunks did not complete within timeout")
            for future in not_completed:
                logger.warning(f"  - Chunk not completed")
        else:
            logger.info("All chunks completed successfully")
    
    final_count = db_connection.execute("SELECT COUNT(*) FROM Address").fetchone()[0]
    total_time = time.time() - start_time
    logger.info(f"Transformed {final_count} addresses in {format_time(total_time)} using parallel processing")


def process_chunk_parallel(db_path: str, run_tag: str, chunk_num: int, total_chunks: int, 
                          offset: int, chunk_size: int, total_count: int, start_time: float) -> None:
    """Process a single chunk in parallel."""
    chunk_start_time = time.time()
    
    logger.info(f"Starting parallel chunk {chunk_num + 1}/{total_chunks} (offset: {offset:,})")
    ...
    try:
        ...
        logger.info(f"Parallel Chunk {chunk_num + 1}/{total_chunks} COMPLETED: {processed_count:,}/{total_count:,} ({progress_percent:.1f}%) - Rows: {rows_affected:,} - Chunk Time: {format_time(chunk_elapsed)}, Total: {format_time(total_elapsed)}")
