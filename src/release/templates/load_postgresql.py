...
def load_sql_file(conn, filepath, description, chunk_size=8192, strip_conflict=False):
    ...
    try:
        ...
        if file_size < 10 * 1024 * 1024:
            ...
            with open(filepath, "r", encoding="utf-8") as f:
                while True:
                    ...
                    if progress >= last_progress + 10:
                        # Calculate ETA for reading phase
                        ...
                        print(f"   Reading... {progress}% - {eta_str}", end="\r")
            ...
            # Debug: Log if we're in performance mode
            if strip_conflict:
                print(f"   ℹ Performance mode enabled - stripping ON CONFLICT clauses")
            ...
            with open(filepath, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if stripped.endswith(";"):
                        ...
                        # Strip ON CONFLICT for performance if requested
                        if strip_conflict:
                            # Debug: Show first statement transformation
                            if statements_executed + statements_skipped == 0:
                                original_length = len(statement)
                            statement = statement.replace(" ON CONFLICT DO NOTHING", "")
                            if statements_executed + statements_skipped == 0:
                                print(
                                    f"   ℹ First statement: {len(statement)} chars (was {original_length}, stripped {original_length - len(statement)} chars)"
                                )
                        ...
                        if progress >= last_progress + 5:
                            ...
                            print(
                                f"   Progress: {progress}% ({statements_executed:,} executed, {statements_skipped:,} skipped) - {eta_str}",
                                end="\r",
                            )
                            last_progress = progress
                        ...
                        try:
                            ...
                            # When strip_conflict is True, we shouldn't get ANY errors
                            if strip_conflict:
                                # Track error types
                                ...
                                error_counts[error_key] = (
                                    error_counts.get(error_key, 0) + 1
                                )

                                # Log first occurrence of each error type
                                if error_counts[error_key] == 1:
                                    print(f"\n   ⚠ New error type: {error_type}")
                                    print(f"      {error_msg}")
                                statements_skipped += 1
                            else:
                                # With ON CONFLICT, some errors are expected
        ...
        # Warn if records were skipped in performance mode
        if strip_conflict and statements_skipped > 0:
            print(
                f"\n⚠ Warning: {statements_skipped:,} statements were skipped even in performance mode."
            )
            print(f"   This is unexpected when using --drop-database or --clean.")
            print(f"\n   Error summary:")
